...
export function registerPrompts(server: McpServer) {
  server.registerPrompt(
    "api_overview_ja",
    {
      title: "API 概要の要約（日本語）",
      description:
        "与えられた OpenAPI 仕様の要点（サーバー、主要リソース、代表的なエンドポイントなど）を日本語で簡潔に要約します。",
      argsSchema: {
  ...
  server.registerPrompt(
    "api_request_drafter_ja",
    {
      title: "API リクエスト下書き（日本語）",
      description:
        "与えられたオペレーション情報から、実用的なリクエスト例（curl / fetch）を日本語コメント付きで下書きします。",
      argsSchema: {
        operationJson: z
          .string()
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text:
                "あなたは API クライアント実装の支援を行うエンジニアです。日本語コメントを入れて、実行可能に近いリクエスト例を作ってください。"
            }
          },
          {
            role: "user",
